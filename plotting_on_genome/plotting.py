...
def main(genome_file, annot_file, locus_file, output_prefix):
    ...
    geneMappingAlignment["insert_length"] = geneMappingAlignment["insert_length"].str.replace("?", "nan", regex=False).astype(float)
    #delete all inserts that are larger than 1e6: nonsense inserts.
    geneMappingAlignment = geneMappingAlignment[np.absolute(geneMappingAlignment["insert_length"]) < 1e6].reset_index()
    ...
    mappedHitContigs_dict = dict((k,j) for j,k in enumerate(mappedHitContigs)) #create dictionary
    ...
    for i in range(len(genomeContigLength)):
        ...
        if genomeContigNames[i] in inter: #label only contigs that got mapping
            contigMaps.append(GraphicFeature(start=contigStart, end=contigEnd, strand=+1,
                                             color=contigColors["blue"],label=genomeContigNames[i]))
        else:
            contigMaps.append(GraphicFeature(start=contigStart, end=contigEnd, strand=+1,
                                             color=contigColors["blue"]))
                
        genomeContigStart.append(contigStart) #save contig start and end in the "whole genome"
    ...
    for i in range(len(geneMappingAlignment)):
        ...
        mappingStart = genomeContigStart[genomeContigNames.index(f_contig)] + geneMappingAlignment["f_start"][i]
        mappingEnd = genomeContigStart[genomeContigNames.index(f_contig)] + geneMappingAlignment["r_start"][i]
        # only map if length is <10000
        if abs(mappingStart-mappingEnd)<10000:
            contigMaps.append(GraphicFeature(start=mappingStart, end=mappingEnd, strand=+1,
                                             color=contigColors["green"],label=geneMappingAlignment["Name"][i]))
    # plot contigs
    ...
    geneMappingAlignment["new_end"] = geneMappingAlignment["seq_end"] + 5500 #modified end of the sequence so that we can plot the area on the genome around the sequence
    geneMappingAlignment["seg_length"] = geneMappingAlignment["new_end"] - geneMappingAlignment["new_start"] 
    ...
    for val in mappedHitContigs:
        ...
        genes_split = geneMappingSubset["Genes"].str.split("\), ").explode().apply(lambda x : x.split(" ")[0] if type(x) == str else "")
        ...
        for i, row in geneMappingSubset.iterrows():
            ...
            cond1 = (geneLocationsSubset["start"] >= row["seq_start"]) | (geneLocationsSubset["end"] >= row["seq_start"]) 
            cond2 = (geneLocationsSubset["end"] <= row["seq_end"]) | (geneLocationsSubset["start"] <= row["seq_end"]) 
            new_subset = geneLocationsSubset[cond1 & cond2]
    
            contigMappingFragments = []
            for j, row2 in new_subset.iterrows(): 
                if row2["start"] < row2["end"]:
                    contigMappingFragments.append(GraphicFeature(start=row2["start"], end=row2["end"], strand=+1, color=contigColors["green"], label=row2["locus_tag"]))
                else:
                    contigMappingFragments.append(GraphicFeature(start=row2["end"], end=row2["start"], strand=-1, color=contigColors["green"], label=row2["locus_tag"]))
        
            fig, axs = plt.subplots(2, 1, figsize=(8, 6))
        
            record = GraphicRecord(sequence_length = row["seg_length"], first_index=row["new_start"], features=contigMappingFragments)
            ...
            cond1 = (geneMappingSubset["f_start"] >= row["new_start"]) | (geneMappingSubset["r_start"] >= row["new_start"])
            cond2 = (geneMappingSubset["r_start"] <= row["new_end"]) | (geneMappingSubset["f_start"] <= row["new_end"])
            ...
            for j, row2 in new_subset.iterrows(): 
                if row2["f_start"] < row2["r_start"]:
                    contigMappingFragments.append(GraphicFeature(start=row2["f_start"], end=row2["r_start"], strand=+1, color=contigColors["green"], label=row2["Name"]))
                else:
                    contigMappingFragments.append(GraphicFeature(start=row2["r_start"], end=row2["f_start"], strand=-1, color=contigColors["green"], label=row2["Name"]))
