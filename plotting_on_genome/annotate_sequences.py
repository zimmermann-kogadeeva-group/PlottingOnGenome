...
def get_row(name, section):
    ...
    row = {
        ...
        "f_start": -1,
        "f_end": -1,
        "f_alignment_subject": None,
        "r_start": -1,
        "r_end": -1,
        "r_alignment_subject": None,
    }
    ...
    if f_cond.any():
        ...
        row.update(section[f_cond].iloc[0][old_cols].rename(index=new_cols).to_dict())
    ...
    if r_cond.any():
        ...
        row.update(section[r_cond].iloc[0][old_cols].rename(index=new_cols).to_dict())
...
def get_gene_text(df_annot, start, end, alignment_subject):
    ...
    gene_text = [
        f"{x} ({y}, {z:.2f})"
        for x, y, z in zip(subset["locus_tag"], subset["strand"], percents)
    ]
...
def main(locus_file, blast_file, outputfile):
    ...
    alignment["direction"] = alignment["query"].apply(
        lambda x: "F" if x.endswith("_F") else "R"
    )
    alignment["name"] = alignment["query"].str.replace("_F", "").str.replace("_R", "")

    res = pd.DataFrame([get_row(name, x) for name, x in alignment.groupby(by="name")])
