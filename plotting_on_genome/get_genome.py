...
def get_all_contigs(genome_ids, fname_prefix):
    records = [
        Entrez.efetch(db="nucleotide", id=x, rettype="fasta", retmode="text").read()
...
def get_all_feature_tables(genome_ids, fname_prefix):
    for genome_id in tqdm(genome_ids):
        record = Entrez.efetch(
            db="nucleotide", id=genome_id, rettype="ft", retmode="text"
        ).read()[:-1]
...
def open_feature_table(filename):
    ...
    with open(filename) as fh:
        ...
        for i, line in enumerate(fh):
            if not line.startswith("\t"):
                ...
                line = line.split("\t")
                row = {
                    "genomic_accession": cur_accession,
    ...
    df_res = df_res.reindex(
        df_res.columns.union(cols, sort=False), axis=1, fill_value=np.nan
    )
...
def main(search_term, email, fname_prefix):
    ...
    df_annot = pd.concat(
        [open_feature_table(f"{fname_prefix}_{x}.ft") for x in genome_ids],
        ignore_index=True,
    )
